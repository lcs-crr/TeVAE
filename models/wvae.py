...
class WVAE(tf.keras.Model):
    ...
    def loss_fn(self, X, Xhat, Xhat_mean, Xhat_logvar, Z_mean, Z_logvar):
        ...
        return -tf.reduce_sum(loglik_loss, axis=1), kl_loss
    ...
    def test_step(self, X):
        if isinstance(X, tuple):
            X = X[0]
        # Forward pass through encoder
        Z_mean, Z_logvar, Z, _ = self.encoder(X, training=False)
        # Forward pass through decoder
    ...
    def call(self, inputs, **kwargs):
        Z_mean, Z_logvar, Z, _ = self.encoder(inputs, **kwargs)
...
class WVAE_Encoder(tf.keras.Model):
    def __init__(self, seq_len, latent_dim, features, seed):
        super(WVAE_Encoder, self).__init__()
...
class WVAE_Decoder(tf.keras.Model):
    def __init__(self, seq_len, latent_dim, features, seed):
        super(WVAE_Decoder, self).__init__()
